...
const TeamMembersManager: React.FC = () => {
  ...
  const handleDeleteUser = async (user: User) => {
    ...
    console.log('Delete attempt:', { userId, currentUserId, user });
    ...
    setDeleteConfirm({ ...user, id: userId });
  };
  ...
  const handleRoleChange = async (userId: string, newRole: string) => {
    if (!userId) {
      setError('User ID is missing. Cannot update role.');
      return;
    }
    ...
    try {
      ...
      setUsers(users.map(user => {
        ...
        return userIdToCheck === userId ? { ...user, role: newRole as User['role'] } : user;
      }));
    ...
    <div className="team-members-manager">
      ...
      <div className="users-table-container">
        <table className="users-table">
          ...
          <tbody>
            {users.map(user => {
              // Handle both 'id' and '_id' properties from backend
              const userId = user.id || user._id;
              const currentUserId = currentUser?.id || currentUser?._id;
              
              return (
                <tr key={userId} className={userId === currentUserId ? 'current-user' : ''}>
                  <td>{user.name}</td>
                  <td>{user.email}</td>
                  <td>
                    <select
                  ...
                  <td>
                    {userId === currentUserId ? (
                      ...
                      <button
                        onClick={() => handleDeleteUser({ ...user, id: userId })}
