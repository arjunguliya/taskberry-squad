...
import { getPendingUsers, approveUser, rejectUser, getAllUsers, getActiveUsers, getSupervisors, getManagers } from "@/lib/dataService.ts";
import EnhancedApprovalForm from "./team/EnhancedApprovalForm";
...
<Dialog open={approvalDialog.open} onOpenChange={(open) => 
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Enhanced Approval Dialog - Using our new component */}
      <Dialog open={approvalDialog.open} onOpenChange={(open) => 
        setApprovalDialog(prev => ({ ...prev, open }))
      }>
        <DialogContent className="max-w-2xl">
          {approvalDialog.user && (
            <EnhancedApprovalForm
              pendingUser={approvalDialog.user}
