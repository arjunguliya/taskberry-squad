...
// Get assignable users from backend
export const getAssignableUsers = async (currentUserId: string): Promise<User[]> => {
  try {
    console.log('Getting assignable users from backend...');
    
    const response = await fetch(`${API_BASE_URL}/api/tasks/assignable-users`, {
      headers: getAuthHeaders()
    });

    if (response.ok) {
      const users = await response.json();
      console.log('Assignable users from backend:', users);
      ...
      console.error('Failed to fetch assignable users:', response.status);
      return [];
    }
  } catch (error) {
...
export const canEditTask = async (taskId: string, currentUserId: string): Promise<boolean> => {
  try {
    // Try to fetch the task - if we can fetch it, we can view it
    const task = await getTaskById(taskId);
    if (!task) return false;
    
    const currentUser = getCurrentUser();
    
    // Rule 1: Creator of task can edit (when we have createdBy field)
    if (task.createdBy === currentUserId) {
      return true;
    }
    
    // Rule 2: Manager can edit tasks created by their team
    if (currentUser.role === 'manager' || currentUser.role === 'super_admin') {
      return true; // Backend will validate specific permissions
    }
...
export const canReassignTask = async (taskId: string, currentUserId: string): Promise<boolean> => {
  try {
    ...
    // Rule 1: Creator of task can reassign (when we have createdBy field)
    const task = await getTaskById(taskId);
    if (task && task.createdBy === currentUserId) {
      return true;
    }
    
    // Rule 2: Manager can reassign tasks
    if (currentUser.role === 'manager' || currentUser.role === 'super_admin') {
      return true;
    }
