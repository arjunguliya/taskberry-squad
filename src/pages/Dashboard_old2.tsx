...
import { getTasksForTeam, getTeamMembers, updateTaskStatus, getActiveUsers } from "@/lib/dataService.ts";
...
export default function Dashboard() {
  ...
  useEffect(() => {
    const loadDashboardData = async () => {
      try {
        console.log('Dashboard: Loading data for user:', currentUser?.name, 'Role:', currentUser?.role);
        ...
        console.log('Dashboard: All active users:', activeMembers);
        
        // Filter team members based on current user's role (same logic as Team.tsx)
        ...
        console.log('Dashboard: Filtered team members:', filteredMembers);
        setTeamMembers(filteredMembers);
        
        // For now, use empty array for tasks since the backend doesn't have these endpoints yet
        // TODO: Replace with actual API calls when backend is ready
        const tasks: Task[] = [];   // await getTasksForTeam(currentUser.id);
        setTeamTasks(tasks);
  ...
  // Calculate team statistics (same as Team.tsx)
  const getTeamStats = () => {
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        >
          ...
          <CardContent>
            ...
            <div className="mt-2">
              <AvatarGroup users={teamMembers.slice(0, 5)} />
            </div>
          </CardContent>
        </Card>
        <Card 
        ...
        <DialogContentComponent className="sm:max-w-[600px]">
          <DialogHeaderComponent>
            <DialogTitleComponent className="flex items-center gap-2">
              ...
              Team Members ({teamStats.totalMembers})
            </DialogTitleComponent>
          </DialogHeaderComponent>
          <TeamMembersList members={teamMembers.filter(member => member.role !== 'super_admin')} />
